...
def detect_injection(user_input: str) -> bool:
    ...
    INJECTION_PATTERNS = [
        ...
        r"disregard (all )?(prior|previous|above) directives?",
        r"system prompt",
        r"override your (system )?prompt",
        r"bypass (your )?(instructions?|security)",
        r"bỏ qua (mọi )?(lệnh|hướng dẫn) (trước đó|trước)",  # Vietnamese
        r"tiết lộ (mật khẩu|thông tin|system)",  # Vietnamese
    ]
...
def topic_filter(user_input: str) -> bool:
    ...
    # Check if input contains any allowed banking topics
    ...
    # If no allowed topic found, block it
    ...
    async def on_user_message_callback(
        ...
        *,
        ...
        if topic_filter(text):
            self.blocked_count += 1
            return self._block_response(
                "I can only help with banking-related questions such as "
                "accounts, transactions, loans, and interest rates. "
